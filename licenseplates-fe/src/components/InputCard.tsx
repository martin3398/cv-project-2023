...
const InputCard = ({ image, setImage }: Props) => {
  ...
  return (
    <Card bg="secondary" text="white" style={{ width: '100%' }}>
      <Card.Body>
        <Card.Text className="card-title">Input Image</Card.Text>
      </Card.Body>

      {image && <Card.Img variant="bottom" src={image} width="50%" />}

      <Card.Body>
        <input type="file" ref={hiddenFileInput} onChange={handleUpload} style={{ display: 'none' }}></input>
        <Button className="float-right bt-all" onClick={handleClick}>
          Upload
        </Button>
      </Card.Body>
    </Card>
